...
export const getEvents = (cnt: number, startAt: string) => {
  const params = Object.assign({cnt}, startAt.length !== 0 ? {startAt} : {});
...
export const joinEvent = (ticketId: number, orderTicketNum: number) =>
  reserveAxios.post('/users/reserve', {ticketId, orderTicketNum}, {headers: {Accept: 'application/json'}});

export const checkJoinEvent = (ticketId: number, orderTicketNum: number) =>
  reserveAxios.post('/users/reserve/check', {ticketId, orderTicketNum}, {headers: {Accept: 'application/json'}});
