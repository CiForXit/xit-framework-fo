...
export const inputForm: React.FC = () => {
  return (
    <FormItem
      ...
      labelExplanation={text('labelExplanation', '주제를 잘 나타내는 멋진 제목을 입력해주세요.')}
      captionContent={text('captionContent', '제목을 입력해주세요.')}
      invalid={boolean('invalid', false)}
      required={boolean('required', false)}
      direction={select(directions.label, directions.options, directions.defaultValue)}>
...
export const TUIEditorForm: React.FC = () => {
  return (
    <FormItem
      label={text('label', '내용')}
      labelExplanation={text('labelExplanation', '행사의 상세한 내용을 알리는 글을 작성해주세요.')}
      invalid={boolean('invalid', false)}
      direction="column">
      <TuiEditor onChange={() => {}} placeholder={text('placeholder', '내용을 입력해주세요.')} />
    </FormItem>
  );
};
