  ...
  return (
    <>
      ...
      <S.TicketContentContainer>
        <S.TicketContentWrapContainer data-testid={'ticket-container'} disabled={disableState.status}>
          <S.TicketPriceWrapper>
            <Price separated>{price}</Price>
          </S.TicketPriceWrapper>
          <S.TicketName>{`${name} ${count ? `* ${count}` : ''}`}</S.TicketName>
          <S.TicketDesc>{desc}</S.TicketDesc>
          {leftCnt !== -1 && <IconLabel icon={<FaTicketAlt size={'1.5rem'} />} labelContent={`${leftCnt}개 남음`} />}

          <IconLabel icon={<FaCheck size={'1.5rem'} />} labelContent={`1인당 ${maxCntPerPerson}개 구입 가능`} />
          <IconLabel icon={<FaRegCalendarAlt size={'1.5rem'} />} labelContent={disableState.label} />
