import React, { ReactElement, useState, useRef, RefObject, useCallback } from 'react';
import * as S from './style';
import { ONLY_IMG_FILE_INFO, IMG_UPLOAD_INFO, FILE_NOT_FOUND_ERROR_INFO, EXCEEDED_FILE_SIZE_LIMIT } from 'commons/constants/string';
...
function readFileOfInput(inputRef: RefObject<HTMLInputElement>, maxSize?: number): Promise<{ data: string; file: File }> {
  return new Promise((resolve, reject) => {
    if (!inputRef.current || !inputRef.current.files) return reject(new Error(FILE_NOT_FOUND_ERROR_INFO));
    ...
    fileReader.onload = (e: ProgressEvent<FileReader>): void => {
      if (!e.target || !e.target.result) return reject(new Error(FILE_NOT_FOUND_ERROR_INFO));
...
function ImgSelector({ onChange, height = '20rem', maxSize }: Props): ReactElement {
  ...
  return (
    <S.Container {...{ background, height }}>
      <S.File type="file" accept="image/*" onChange={onChangeFile} ref={inputRef} />
