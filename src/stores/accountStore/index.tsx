import React, {createContext, useReducer, Dispatch, SetStateAction, useEffect, useState} from 'react';
...
export const UserAccountState = createContext<AccountState>(defaultAccountState);
...
function AccountStoreProvider({children}: {children: React.ReactElement}) {
  const [accountState, accountDispatcher] = useReducer<AccountReducer>(accountReducer, defaultAccountState);
  ...
  useEffect(() => {
    ...
    if (type === FAILURE) {
      if (err && err.response) dispatchValue = {...err.response.data, isLogin: false};
  ...
  return (
    <UserAccountAction.Provider value={{accountDispatcher, setLoginState}}>
      <UserAccountState.Provider value={accountState}>{children}</UserAccountState.Provider>
